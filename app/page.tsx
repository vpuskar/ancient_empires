...
import { EmpireCardTracker } from '@/components/analytics/EmpireCardTracker';
...
function EmpireCard({ empire }: { empire: EmpireConfig }) {
  return (
    <EmpireCardTracker empireSlug={empire.slug} empireName={empire.name}>
      <Link
        href={`/${empire.slug}`}
    ...
    <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-2xl">
        ...
        <p className="text-zinc-500 text-center mb-10">
          Explore the great empires of history
        </p>
